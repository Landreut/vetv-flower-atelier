...
export function validateOrder(body: unknown) {
  ...
  const bouquet = bouquets.find((item) => item.id === string('bouquetId'));
  ...
  if (!bouquet) return { error: 'Выберите букет из коллекции.' } as const;
  ...
  return { order: { id, bouquet, name, phone, date: date || null, wishes } } as const;
}
